...
\renewcommand\BackgroundPicture{
  \begin{tikzpicture}
    ...
    \fill[white] (74.9,110) rectangle (85,119);
    \node(qr) at (77.4,111.5) {\includegraphics[scale=0.23]{ldl-frog.pdf}};
  \end{tikzpicture}
}
...
\title{
  \vspace{-7mm}\LARGE{\textbf{Automating Ad hoc Data Representation Transformations}\bkpspace}\vspace{-3mm}
}
\author{
  Vlad Ureche$^1$ \kern1em Aggelos Biboudis$^2$ \kern1em Yannis Smaragdakis$^2$ \kern1em Martin Odersky$^1$\\
  École polytechnique fédérale de Lausanne, Switzerland$^1$ \kern1em University of Athens$^2$\\
  \texttt{\frenchspacing{}\{first.last\}@epfl.ch$^1$
    \frenchspacing{}biboudis@di.uoa.gr
    \frenchspacing{}smaragd@di.uoa.gr \; {\small \texttt{\frenchspacing{}scala-ldl.org}}}
  \vspace{-13mm}
}
...
\begin{tikzpicture}
  ...
  \ifthenelse{\equal{\template}{1}}{
    ...
    \titleblock[(-3, -0.5)]{47}{1.5}
  }

  %% a logo can be added to the title block
  %% #1 - anchor relative to the title block, #2 - shift, #3 - width, #3 - file name
  % \addlogo[south west]{(-5,0.0)}{90mm}{ldl-frog.pdf}
